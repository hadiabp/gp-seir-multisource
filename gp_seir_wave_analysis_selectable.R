...
#   - Beta-initialization sensitivity per wave
#   - Start-date sensitivity (wave 1)
#   - Approximate profile-likelihood refits per wave
...
#
# All outputs are prefixed with the selected window label (wave1, interwave, or wave2).
...
#   Rscript gp_seir_wave_analysis.R --wave=wave1
#   Rscript gp_seir_wave_analysis.R --waves=wave1,wave2
#
...
# By default the script runs all three windows. You can override this from the
# command line, e.g.
#   Rscript gp_seir_wave_analysis.R --wave=wave1
#   Rscript gp_seir_wave_analysis.R --waves=wave1,wave2
...
for (arg in args) {
  if (grepl("^--wave=", arg)) {
    RUN_WAVES <- sub("^--wave=", "", arg)
  } else if (grepl("^--waves=", arg)) {
    RUN_WAVES <- strsplit(sub("^--waves=", "", arg), ",")[[1]]
  }
}
RUN_WAVES <- unique(trimws(RUN_WAVES))
if (!all(RUN_WAVES %in% names(WAVE_DEFS))) {
  stop("Unknown wave selection: ", paste(setdiff(RUN_WAVES, names(WAVE_DEFS)), collapse = ", "),
       ". Valid options are: ", paste(names(WAVE_DEFS), collapse = ", "))
}
...
fit_gp_seir <- function(df, date_start, date_end,
  ...
  # Optional fixed scaling parameters for approximate profile-likelihood runs.
  # Allowed names: rho, pH, pICU, alphaR.
  ...
  # For initialization only, we ignore reporting delay and invert the
  # instantaneous case link to construct a rough proxy for E(t).
  ...
  if (length(fixed_params) > 0) {
    fixed_idx <- unname(param_idx[names(fixed_params)])
  }
...
# ══════════════════════════════════════════════════════════════════════════════
# SECTION 8: BETA-INITIALIZATION SENSITIVITY PER WAVE
# Note: only the beta initialization changes here. The state initialization
# and non-beta starting values are held fixed.
# ══════════════════════════════════════════════════════════════════════════════

cat("\n================================================================\n")
cat("STEP 4: Beta-initialization sensitivity per wave\n")
cat("================================================================\n")
...
if (length(start_rows) > 0) {
  ...
  cat("  Skipped: wave1 not selected.\n")
}


# ══════════════════════════════════════════════════════════════════════════════
# SECTION 10: APPROXIMATE PROFILE-LIKELIHOOD REFITS PER WAVE
# ══════════════════════════════════════════════════════════════════════════════

cat("\n================================================================\n")
cat("STEP 6: Approximate profile-likelihood refits per wave\n")
cat("================================================================\n")

for (wname in intersect(c("wave1", "wave2"), RUN_WAVES)) {
  wdef <- WAVE_DEFS[[wname]]
  cat(sprintf("\n  === %s ===\n", wdef$label))
  ...
  for (ps in param_specs) {
    ...
    for (gv in grid_vals) {
      ...
      fit_p <- tryCatch({
        # Approximate profile-likelihood refit with the chosen scaling parameter
        # held fixed in the outer loop.
        fit_gp_seir(FULL_DF, wdef$fit_start, wdef$fit_end,
                    seed = 400 * wdef$id + round(gv * 1000),
                    n_warm = 3, n_cold = 2, verbose = FALSE,
                    fixed_params = setNames(list(gv), ps$name))
